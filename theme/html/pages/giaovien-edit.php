<?php 
	...
	if($_SERVER['REQUEST_METHOD'] === 'POST'){
		...
		$idsv = $_GET['id'];
		$query = $fluent->update('giao_vien')->set($values)->where('id_gv', $idsv);
		$query->execute();
		header("Location:http://localhost/qlsv/giao-vien/");
	}	
	if($_SERVER['REQUEST_METHOD'] === 'GET'){
		$idsv = $_GET['id'];	
		$query = $fluent->from('giao_vien')->where('id_gv = ?', $idsv);	
		foreach ($query as $sv) {		
?>
<form class="form-horizontal row-fluid" method="post">
		...
		<div class="control-group" style="display: none;">
			...
			<div class="controls">
				<input type="text" name="ma_sinh_vien" id="ma_sinh_vien" value="<?=$sv['ma_gv'] ?>" class="span8" disabled="">
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="ho_ten">Họ và tên</label>
			<div class="controls">
				<input type="text" name="ho_ten" id="ho_ten" value="<?=$sv['hoten_gv']?>" class="span8">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input type="date" name="old" class="span8 tip" value="<?=$sv['ngaysinh_gv']; ?>">
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				...
				<label class="radio inline">
					<input type="radio" name="optionsRadios" id="optionsRadios1" value="0" <?php if($sv['gioitinh'] == '0') echo 'checked="true"'; ?> >
					Nam
				</label> 
				<label class="radio inline">
					<input type="radio" name="optionsRadios" id="optionsRadios2" value="1" <?php if($sv['gioitinh'] == '1') echo 'checked="true"'; ?>>
					Nử
				</label> 				
				</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<input type="text" name="so_dien_thoai" id="so_dien_thoai" value="<?=$sv['dienthoai']; ?>" class="span8 tip">
			</div> 
		</div>
		<div class="control-group">
			<label class="control-label" for="sv_email">Email</label>
			<div class="controls">
				<input type="text" name="sv_email" id="sv_email" value="<?=$sv['email']; ?>" class="span8 tip" >
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="dia_chi">Địa chỉ</label>
			<div class="controls">
				<textarea class="span8" name="addr" rows="5"><?=$sv['diachi']; ?> </textarea>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="noi_sinh">Nơi sinh</label>
			<div class="controls">
				<input type="text" name="noi_sinh" id="noi_sinh" value="<?=$sv['noisinh']; ?>" class="span8">			
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="ho_khau">Hộ khẩu</label>
			<div class="controls">
				<textarea class="span8" name="hokhau" rows="5"><?=$sv['hokhau']; ?></textarea>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="que_quan">Quê quán</label>
			<div class="controls">
				<input type="text" name="que_quan" id="que_quan" value="<?=$sv['quequan']; ?>" class="span8">			
			</div>
		</div>		
		<div class="control-group">
			<label class="control-label" for="ngay_vao_doan">Ngày vào đoàn</label>
			<div class="controls">
				<input type="date" name="ngayvaodoan" value="<?=$sv['ngayvaodoan']?>" class="span8 tip">
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="ngay_vao_dang">Ngày vào đảng</label>
			<div class="controls">
				<input type="date" name="ngay_vao_dang" value="<?=$sv['ngayvaodang']?>" class="span8 tip">
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<select name="bo_mon" tabindex="1" data-placeholder="Select here.." class="span8">
					...
					<?php 
						...
						foreach ($list_adr as $row) {
					?>
					<option value="<?=$row['id_mh'] ?> <?php if($row['id_mh'] == $sv['bomon']) echo 'selected'; ?>" ><?=$row['ten_mh']?></option>		
					<?php } ?>			
				</select>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<input type="text" name="bang_cap" id="bang_cap" value="<?=$sv['bangcaonhat']?>" class="span8">				
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="ngaybatdau">Ngày bắt đầu</label>
			<div class="controls">
				<input type="date" name="ngaybatdau" value="<?=$sv['ngaybatdau']?>" class="span8 tip">
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="ngayketthuc">Ngày kết thúc</label>
			<div class="controls">
				<input type="date" name="ngayketthuc" value="<?=$sv['ngayketthuc']?>" class="span8 tip">
			</div>
		</div>
		<div class="control-group">
			<div class="controls">
